#!/bin/bash
# Run LLM Debate (2 agents, 5 rounds) on GAIA level1 → level2 → level3 sequentially.
#
# Usage:
#   nohup ./scripts/run_llm_debate_gaia_all_levels_5rounds.sh > logs/llm_debate_gaia/all_levels_5r_<ts>.log 2>&1 &
#   MODEL_NAME=gpt-4.1 CONCURRENCY=10 ./scripts/run_llm_debate_gaia_all_levels_5rounds.sh
...
DEBATE_ROUNDS="${DEBATE_ROUNDS:-5}"
DEBATE_AGENTS="${DEBATE_AGENTS:-2}"
...
run_level() {
  local level="$1"
  local data_path="$2"
  local log_file="$LOG_DIR/llm_debate_gaia_level${level}_${DEBATE_AGENTS}a${DEBATE_ROUNDS}r_${MODEL_NAME}_${TIMESTAMP}.log"

  echo ""
  echo "============================================================"
  echo "  Level ${level} — $(date '+%Y-%m-%d %H:%M:%S')"
  echo "  Data:        $data_path"
  echo "  Concurrency: $CONCURRENCY  |  Agents: $DEBATE_AGENTS  |  Rounds: $DEBATE_ROUNDS"
...
echo "====================================================="
echo "LLM Debate GAIA all levels  |  ${DEBATE_AGENTS} agents  |  ${DEBATE_ROUNDS} rounds"
