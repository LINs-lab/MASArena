...
def print_agent_info(agent: 'BenchEnhancedAgent', score: float = None):
...
class EvoAgent(AgentSystem):
    ...
    async def _crossover(self, parent1: BenchEnhancedAgent, parent2: BenchEnhancedAgent) -> BenchEnhancedAgent: # <--- 引用更新
        """
    ...
    async def _mutation(self, parent: BenchEnhancedAgent) -> BenchEnhancedAgent: # <--- 引用更新
        """
    ...
    async def _run_agent_task(self, agent: BenchEnhancedAgent, problem_text: str, problem: Dict[str, Any]) -> None: # <--- 引用更新
        """
