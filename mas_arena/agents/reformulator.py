...
def truncate_observation(step_dict: dict) -> None:
    if 'observations' not in step_dict:
        pass
    else:
        observations = step_dict.pop('observations', None)
        if observations and len(observations) > MAX_OBSERVATION_LENGTH:
            observations = observations[:MAX_OBSERVATION_LENGTH] + " [truncated]..."
        step_dict['observations'] = observations
