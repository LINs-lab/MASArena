...
# Prompt Templates for EvoMAC - Organized by role/agent similar to MetaGPT
PROMPTS = {
    "cto_system": """EvoMAC is a software company powered by multiple intelligent agents, such as chief executive officer, chief human resources officer, chief product officer, chief technology officer, etc, with a multi-agent organizational structure and the mission of 'changing the digital world through programming'.
...
4) Your task should not include anything related to testing, writing document or computation cost optimizing.""",
    ...
    "initial_coder_system": """EvoMAC is a software company powered by multiple intelligent agents, such as chief executive officer, chief human resources officer, chief product officer, chief technology officer, etc, with a multi-agent organizational structure and the mission of 'changing the digital world through programming'.""",

    "programmer_system": """You are a professional programmer at EvoMAC. Your task is to implement code according to the given specifications.
...
5. No placeholder code (such as 'pass' in Python)""",
    ...
    "test_organizer_system": """You are a Test Manager at EvoMAC. Your goal is to organize a testing team to complete the function testing task.
...
3) The task number should be as small as possible. Only one task is also acceptable.""",
    ...
    "test_engineer_system": """You are a Test Engineer at EvoMAC. Your task is to write test cases according to the function specifications.
...
4) According to example test case in the Task description, please only write these test cases to locate the bugs. You should not add any other testcases by yourself except for the example test case given in the Task description""",
    ...
    "updater_system": """You are a Senior Developer at EvoMAC. Your task is to organize a programmer team to solve current issues in the code.
...
{format_prompt}""",
    ...
    "sub_code_complete": """Here is a function completion task:
Task: "{task}".
Programming Language: "{language}"
The implemention of the task(source codes) are: "{codes}"
...
{format_prompt}""",
    ...
    "test_organizing": """According to the function completion requirements listed below: 
...
{format_prompt}""",
    ...
    "test_code_complete": """According to the function completion requirements listed below: 
Task: "{task}".
...
class EvoMAC(AgentSystem):
        ...
        try:
            ...
            for task_name in execution_order:
                if task_name in composition:
                    ...
                    format_prompt = get_format_prompt("humaneval") or ""
                    
                    # Generate subtask completion prompt
                    prompt = TASK_PROMPTS["sub_code_complete"].format(
                        task=query,
                    ...
                    messages = self.format_messages(PROMPTS["programmer_system"], prompt)
    ...
    async def test_bugs(self, test_code: str) -> tuple[bool, str]:
        ...
        try:
            ...
            try:
                # Run tests using subprocess asynchronously
                ...
                try:
                    ...
                    test_report = stdout + stderr
                    
                    return has_bugs, test_report
                except asyncio.TimeoutError:
                    process.kill()
                    return True, "Test execution timeout"
    ...
    async def execute_test_workflow(self, query: str) -> tuple[bool, str]:
        """Execute test workflow and return bug status and reports"""
        try:
            # Get format prompt for testing
            format_prompt = get_format_prompt("humaneval") or ""
            
            # Generate test organization
            test_organizing_prompt = TASK_PROMPTS["test_organizing"].format(
                task=query,
                language=self.language,
                format_prompt=format_prompt
            )
            ...
            messages = self.format_messages(
                PROMPTS["test_organizer_system"],
                test_organizing_prompt
            )
            test_organization = await self.call_llm(messages)
            ...
            for task_name in test_composition:
                ...
                # Generate test code
                test_prompt = TASK_PROMPTS["test_code_complete"].format(
                    task=query,
                    language=self.language,
                    codes=self.codes._get_codes(),
                ...
                messages = self.format_messages(PROMPTS["test_engineer_system"], test_prompt)
                test_response = await self.call_llm(messages)
                ...
                if test_code_match:
                    ...
                    # Run tests
                    has_bugs, test_report = await self.test_bugs(combined_code)
                    if has_bugs:
                        has_any_bugs = True
    ...
    async def run_agent(self, problem: Dict[str, Any], **kwargs) -> Dict[str, Any]:
        ...
        try:
            # Get format prompt for this task
            format_prompt = get_format_prompt("humaneval") or ""
            
            # 1. Initial coding
            initial_coding_messages = self.format_messages(
                PROMPTS["initial_coder_system"],
                TASK_PROMPTS["initial_coding"].format(task=query)
            )
            
            # Convert messages to LangChain format and call async
            langchain_messages = []
            ...
            # Record message with usage metadata (similar to MetaGPT format)
            ai_message = AIMessage(content=initial_coding_codes)
            ai_message.name = 'initial_coder'
            if hasattr(response, 'usage_metadata'):
                ai_message.usage_metadata = response.usage_metadata
            all_messages.append(ai_message)
            
            # 2. Generate workflow organization
            organizing_messages = self.format_messages(
                PROMPTS["cto_system"],
                TASK_PROMPTS["organizing"].format(
                    task=query,
                    language=self.language,
                    codes=self.codes._get_codes(),
                    format_prompt=format_prompt
            ...
            # Record message (similar to MetaGPT format)
            ai_message = AIMessage(content=organization)
            ai_message.name = 'organizer'
            if hasattr(response, 'usage_metadata'):
                ai_message.usage_metadata = response.usage_metadata
            all_messages.append(ai_message)
            
            # 3. Execute workflow and test
            await self.execute_workflow(query)
            has_bug_in_tests, test_reports = await self.execute_test_workflow(query)
            ...
            for i in range(self.iteration - 1):
                ...
                updating_messages = self.format_messages(
                    PROMPTS["updater_system"],
                    TASK_PROMPTS["updating"].format(
                        task=query,
                        codes=self.codes._get_codes(),
                        issues=test_reports,
                        format_prompt=format_prompt
                ...
                # Record message (similar to MetaGPT format)
                ai_message = AIMessage(content=organization)
                ai_message.name = f'updater_iteration_{i+1}'
                if hasattr(response, 'usage_metadata'):
                    ai_message.usage_metadata = response.usage_metadata
                all_messages.append(ai_message)
            ...
            error_message = AIMessage(content=f"EvoMAC execution failed: {str(e)}")
            error_message.name = 'error_handler'
