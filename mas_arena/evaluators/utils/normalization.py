...
def add_files_to_prompt(problem: Dict[str, Any], file_name: str = None):
    script_dir = os.path.dirname(os.path.abspath(__file__))
    relative_path = os.path.join(
        script_dir, "..", "..", "..", "data", "files", "gaia", "validate", file_name
    )
    file_path = Path(os.path.normpath(relative_path))
    problem["files"] = str(file_path)
...
def normalize_problem_keys(
    problem: Dict[str, Any], key_mapping: Dict[str, str], problem_index: int
) -> Dict[str, Any]:
    """
    ...
    if "file_name" in problem and problem["file_name"]:
        ...
        normalized_problem["problem"] = add_files_to_prompt(
            normalized_problem, file_name
        )
